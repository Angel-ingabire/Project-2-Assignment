...
int main(void) {
    ...
    printf("   ");
    for (int j = 0; j < NODES; ++j) printf(" %2c", names[j]);
    printf("\n");
    for (int i = 0; i < NODES; ++i) {
        printf(" %c ", names[i]);
        for (int j = 0; j < NODES; ++j) printf(" %2d", adj[i][j]);
        printf("\n");
    }
    ...
    for (int i = 0; i < NODES; ++i) { parent[i] = i; rankv[i] = 0; }
    ...
    int root = find_set(0);
    int connected = 1;
    for (int i = 1; i < NODES; ++i) if (find_set(i) != root) connected = 0;
    if (!connected) printf("[ERROR] MST does not connect all nodes.\n");
    else printf("[OK] All nodes are connected in MST.\n");

    // 3) Compute MST via Prim's algorithm (for comparison)
    int inMST[NODES];
    int key[NODES];
    int parentp[NODES];
    for (int i = 0; i < NODES; ++i) { inMST[i] = 0; key[i] = 1e9; parentp[i] = -1; }
    key[0] = 0;
    for (int cnt=0; cnt < NODES; ++cnt) {
        int u = -1;
        int best = 1e9;
        for (int v = 0; v < NODES; ++v) if (!inMST[v] && key[v] < best) { best = key[v]; u = v; }
        if (u == -1) break;
        inMST[u] = 1;
